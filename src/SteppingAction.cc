...
// ********************************************************************
//
/// \file electromagnetic/CalSG/src/SteppingAction.cc
...
void SteppingAction::UserSteppingAction(const G4Step* aStep)
{
  ...
  if (endPoint->GetStepStatus() == fGeomBoundary) {
    ...
    G4double sizeY = 0.5*fDetector->GetCalorSizeY();   
    G4double sizeZ = 0.5*fDetector->GetCalorSizeZ();      
    G4double Eflow = endPoint->GetKineticEnergy();
    if(particle == G4Positron::Positron()) Eflow += 2*electron_mass_c2;
    if((std::abs(position.y()) >= sizeY) || (std::abs(position.z()) >= sizeZ))
